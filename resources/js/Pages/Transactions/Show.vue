        ...
        <template #header>
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-gray-600 mt-1" v-if="item">
        ...
        <div class="py-12">
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                    <!-- Item Summary -->
                    <div class="p-6 border-b border-gray-200" v-if="item">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900">{{ item.name }}</h3>
                                <p class="text-sm text-gray-600">Unit: {{ item.unit.name }} ({{ item.unit.symbol }})</p>
                            </div>
                    ...
                    <div class="p-6" v-if="hasTransactions">
                        <h4 class="text-md font-medium text-gray-900 mb-4">Transaction History</h4>
                        <div class="space-y-4">
                            <div 
                            ...
                            >
                                ...
                                <div class="text-right">
                                    <p class="text-lg font-semibold" :class="transaction.type === 'add' ? 'text-green-600' : 'text-red-600'">
                                        {{ transaction.type === 'add' ? '+' : '-' }}{{ parseFloat(transaction.quantity).toFixed(2) }}
                                    </p>
                                    <p class="text-sm text-gray-500">{{ item?.unit.symbol }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
